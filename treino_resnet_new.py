# treino_resnet18_new.py
# Treino ResNet18 com anti-overfitting (augment forte + dropout/regularizacao via head
# + label smoothing + early stopping + fine-tuning em 2 fases), usando Adam.
...
EPOCHS_HEAD = 6        # backbone congelado (treina só a FC)
...
WEIGHT_DECAY = 1e-4
DROPOUT_P = 0.35       # aplicado antes da camada final
LABEL_SMOOTHING = 0.10


PATIENCE = 7           # early stopping por val_loss
UNFREEZE_LAYERS = 2    # quantos "stages" finais descongelar (1..4). Ex: 2 => layer3 e layer4
...
def main():
    ...
    # Fase 1: congela backbone, treina só a FC
    ...
    # Fase 2: fine-tuning (descongela stages finais)
    stages = [model.layer1, model.layer2, model.layer3, model.layer4]
    k = max(0, min(4, UNFREEZE_LAYERS))
    if k > 0:
        for stage in stages[-k:]:
            for p in stage.parameters():
                p.requires_grad = True

    # Garante que a FC segue treinavel
    for p in model.fc.parameters():
        p.requires_grad = True
